...
public class AdventureEntityManager extends EntityManager {
	...
	public void deleteAdventure(Integer adventureId) throws SQLException {
		...
		try {
			theAdventure = this.getAdventureById(adventureId);
			
			this.initConnection();
			
			if (theAdventure != null) {
				// Delete adventure
				String template = "delete from adventure where id=%d";
				String stmt = String.format(template, adventureId);
				this.connection.executeUpdateStatement(stmt);
			}
